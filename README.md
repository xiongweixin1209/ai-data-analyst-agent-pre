...
| 策略分类准确率 | 56.2% | — | — |
| 平均响应时间 | 4077ms | 8459ms | 3580ms |

评估测试集覆盖 8 类查询意图:单表查询、多表联结、聚合统计、时序查询、窗口函数、复杂嵌套、DWS 表查询、边界异常。

**诊断-修复-验证闭环(项目方法论核心)**:

| 阶段 | default EX | zero-shot EX | 备注 |
|---|---|---|---|
| 初始版本 | 82.5% | 85.0% | zero-shot 反超 default 2.5%,反常识 |
| 诊断 | — | — | 失败用例 85.7% 是列名幻觉;few-shot 示例库用 snake_case,Northwind 真实 schema 用 PascalCase,模型从示例学到了错误命名风格 |
| 修复 | — | — | 批量重写 120 条示例的表名+列名为 Northwind 风格(`orders` → `Orders`,`invoice_no` → `OrderID` 等) |
| **修复后** | **97.5%** | 85.0% | **default 反超 zero-shot 12.5 个百分点**,验证三层策略的真实价值 |

这个故事体现了项目的工程方法论:**先建评估基线 → 反直觉数据指向诊断方向 → 定位根因 → 系统性修复 → 数据验证**,而不是凭直觉调参。
